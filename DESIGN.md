...
中性灰画布（`#131313`）+ 提亮蓝主色。层次靠“**卡片比画布亮**”表达，不靠阴影。
不用纯黑（`#000`）+ 纯白（`#fff`）的极端对比，长时间看会累。

> **灰阶必须中性（R=G=B）—— 和浅色同一条规矩。**
> 历史：深色原本是“暖黑”（`#141312` / `#23211f` / `#b4aea4` 这种 R>G>B），
> 屏幕上一看就偏黄，用户反馈“太暖了” → 2026-09-21 全部换成中性灰（亮度与原值对齐，
> 不是靠减亮度去“显得没那么暖”）。
> 语义色（成功/警告/错误）**保留各自色相** —— 它们是“信号”，不是背景。
...
|---|---|---|
| `--canvas` | `#131313` | 页面底色 |
| `--canvas-soft` | `#1b1b1b` | 标题栏底 |
| `--card` | `#222222` | 卡片底（比画布亮 = 层次） |
| `--card-strong` | `#2b2b2b` | 表头、hover 态 |
| `--hairline` | `#383838` | 边框 |
| `--hairline-soft` | `#292929` | 卡片内分隔线 |
| `--ink` | `#fafafa` | 标题 |
| `--body` | `#eaeaea` | 正文（**保证在中性灰上高对比，这是可读性的关键**） |
| `--muted` | `#b2b2b2` | 次级说明 |
| `--muted-soft` | `#888888` | 占位符、时间戳 |
...
顶栏右侧的按钮：`启动服务` / `重启` / `设置`（**图标按钮**）/ 主题开关 ——
主题是低频操作且属于“设置”语义，已搬进设置页（顶栏只留运行状态与常用动作）。

**常驻告警条 `noticebar`**（红/黄两态）的颜色写在自己的规则里，**不要**用深色专属选择器：
历史坑：那里写的是 `body.theme-dark`，而主题是挂在 `<html data-theme>` 上的 ——
两条规则**从未生效**过，于是深色模式下告警条一直在用浅色底（淡红/淡黄），又亮又暖。
...
**图标按钮 `btn-icon`**：只放一个 SVG（如顶栏「设置」的齿轮）。
固定 34×34、与旁边文字按钮**同高**，内联 SVG 用 `currentColor` 跟着主题变色；
`title` + `aria-label` 补上文字语义（不然只剩下一个无名的图标）。

**两段式滑块 `seg2`**（设置 → 外观 → 主题）：`浅色 | 深色`。
两个选项用 `grid` 两列 `1fr` 保证**严格等宽**（不能靠 padding 凑）；选中态由底下那块
`.seg2-thumb` 滑过去表示（`transform` 过渡），文字颜色随之切换。容器上的 `data-theme`
决定滑块位置。

> 为什么不用 `.tabs`/`.tab`：那是顶栏标签条，`.tab:not(:last-child)` 带 `min-width`，
> 两段宽度会不一样（当时的症状是“两个按钮一大一小”）。
