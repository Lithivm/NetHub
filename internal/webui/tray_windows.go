...
// ───────── 优雅退出的信号通道 ─────────
//
// 为什么需要优雅退出：进程被 taskkill 强杀时来不及 NIM_DELETE，Windows 会把托盘图标
// 留成"僵尸"（进程没了、图标还在，鼠标扫过才消失）。所以给一条正规的退出通道，
// 让它走停引擎 → 移除图标 → 关窗。
//
// 为什么用命名事件而**不是**"找到托盘窗口投 WM_*"（早先实现过，已删）：
//  1. nethub 自己是提权进程，普通权限的进程给它投消息会被 UIPI 拦掉（实测 Access is denied）；
//  2. 窗口消息 / 窗口句柄是**会话内**的 —— 服务版跑在 session 0、界面版在用户会话，
//     服务那边根本看不到这个窗口（互斥与接管要靠它请界面版让位）。
//
// 命名事件是内核对象，两个问题都没有。
//
// 用途：重启脚本 `nethub.exe -quit`；服务版请界面版让位（RequestQuitCrossSession）。
//
