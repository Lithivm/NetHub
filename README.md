...
| **路由规则** | ≈ Proxifier 的 **Rules**。**一条规则 = 名字 + 一组目标 + 走哪条链**，可增删改排序；规则自上而下匹配、命中即止，一条规则内任一目标命中即算命中 |
...
routes:                   # 一条规则 = 一个动作挂一组目标（对齐 Proxifier）
  - name: 内网主体链路       # 规则名，可省略
    targets:                # 目标可多个：任一命中即算该规则命中
...
# 规则自上而下匹配，命中即止。一条规则内多个目标共享同一个位置，
# 所以合并相邻同链规则不会改变行为；网段包含关系（宽里有窄）由你自己排先后。
